...
from domain_expert_core import __version__ as CORE_VERSION
from domain_expert_core.packs.models import (
    DomainPack,
    FieldSpec,
    LinkSpec,
    ObjectSpec,
...
def validate_pack(pack: DomainPack) -> None:
    ...
    try:
        req = Requirement(f"domain-expert-core{pack.manifest.core_compat}")
...
def discover_entry_point_packs() -> list[Path]:
    paths: list[Path] = []
    try:
        eps = importlib.metadata.entry_points(group="domain_expert.packs")
    except Exception:
        return paths
    for ep in eps:
        try:
...
def bundled_packs_root() -> Path:
    ...
    # core/domain_expert_core/packs/loader.py → repo packs/ is parents[3]/packs
