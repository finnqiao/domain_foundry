...
class OpenAICompatibleProvider(LLMProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (base_url or os.environ.get("DOMAIN_EXPERT_LLM_BASE_URL") or "https://api.openai.com/v1").rstrip("/")
        self.api_key = api_key or os.environ.get("DOMAIN_EXPERT_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY")
        self.default_model = default_model or os.environ.get("DOMAIN_EXPERT_LLM_MODEL") or "gpt-4o-mini"
...
def get_default_provider(*, cassette_dir: Path | None = None) -> LLMProvider:
    mode = os.environ.get("DOMAIN_EXPERT_LLM", "heuristic").lower()
    if mode in {"off", "heuristic", "0", "false"}:
        inner: LLMProvider = HeuristicProvider()
    else:
        inner = OpenAICompatibleProvider()
    if cassette_dir is not None:
        cassette_mode = os.environ.get("DOMAIN_EXPERT_CASSETTE", "replay")
