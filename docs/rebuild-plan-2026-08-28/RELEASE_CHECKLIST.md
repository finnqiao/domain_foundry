...
| 2 | Two apps for different passions are visibly, structurally different | `python scripts/foundry_difference_gate.py` | red: 2 of 8 checks | |
| 3 | A real fork path end to end, parentage recorded | `python -m pytest tests/e2e-foundry/test_fork_e2e.py -q` | red: 1 of 9 checks | |
| 4 | An out-of-corpus passion, seeded from a spreadsheet, yields an honest app | `python -m pytest tests/e2e-foundry/test_stranger_passion.py -q` | red: 3 of 6 checks | |
| 5 | README matches code: no dead fields, no ghost surfaces | `python scripts/claims_audit.py` | Lane A | |

What each red row is waiting on:

- **Proof 1.** Nothing has recorded a live pipeline run. Record with
...
- **Proof 2.** Two checks fail: the compiled apps carry no `data-region-kind`
  attributes, and only two landmark tag counts differ between two very
  different practices. Both are the compiler's output (Lane B). Topology,
  palette, type stack, both screenshot diffs, axe and the 320px floor pass.
- **Proof 3.** Everything passes except the bundle README, which does not name
  the parent spec. That line belongs in `FoundryCompiler.render_readme`
  (Lane B). The parent is already on the spec, in `remix.parent_spec`, in a
  derivation, and in `build-receipt.json`.
- **Proof 4.** The honesty floor is green: an out-of-corpus passion with no
  seeds and no consent stops with the three paths. The full walk waits on the
  tidepool fixtures under `examples/seed-fixtures/` (Lane E) and on a recorded
  live run.
...
- The rebuild gates are behind `--rebuild-gates` so a deliberately red gate
  cannot block the standing audit. When all four go green, delete the flag from
