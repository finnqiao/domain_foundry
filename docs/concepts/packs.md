...
A **Domain Pack** is the remix surface. It is the single artifact you author (or the
...
- `domain-foundry pack add <path-or-name>` takes a local pack directory, or the
...
- a **private overlay** directory listed in `DOMAIN_FOUNDRY_PACKS_PATH`
  (personal packs can live entirely outside this repo, for example
...
2. **Pip-installed handlers (trusted code).** A pack that outgrows declarative
   ...
   only). That is an explicit choice by the user to install code.
