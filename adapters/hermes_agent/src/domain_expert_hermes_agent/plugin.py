"""hermes-agent plugin registration for the domain_expert harness.
...
from domain_expert_hermes_agent.client import DomainExpertClient
...
You have a domain_expert harness attached. Follow capture-first discipline:
...
1. When the user reports something that happened in one of their tracked life
   ...
   call `domain_expert_capture` with their words *verbatim*. Do not paraphrase,
...
3. When the user amends or contradicts something ("actually it was 80% not
   75%"), call `domain_expert_correct` with their correction sentence. One
   message, one correction — do not re-capture.
4. To answer "what did I…" questions, call `domain_expert_query` (read-only).
5. Surface pending approvals with `domain_expert_review_list`; only resolve one
...
6. To stand up a brand-new tracking domain from a plain-language goal, call
   `domain_expert_new_domain` and relay the wizard's questions to the user.
...
def _resolve_client(ctx: Any, client: DomainExpertClient | None) -> DomainExpertClient:
    ...
    base_url = (
        ...
        or os.environ.get("DOMAIN_EXPERT_URL")
        or "http://127.0.0.1:8787"
    )
    token = _ctx_get(ctx, "token") or os.environ.get("DOMAIN_EXPERT_API_TOKEN")
