...
def _client(workspace):
    ...
    return TestClient(create_app(workspace.home, enable_drain_loop=False))
...
def test_home_starts_empty_then_lists_installed_domains(workspace):
    ...
    # Install two packs (the "add domain" affordance / scripted install).
...
def test_full_walkthrough(workspace):
    ...
    corrected = client.post(
        "/api/correct",
        json={"object_uid": uid, "action": "amend", "fields": {"hydration": 80}},
    ).json()
...
def test_review_queue_drains_to_zero(workspace):
    ...
    client.post(
        "/api/capture",
        json={"text": "started a rye starter today", "channel": "web"},
    )
    client.post(
        "/api/capture",
        json={"text": "fed the wheat starter this morning", "channel": "web"},
    )
...
def test_move_and_merge_corrections_no_privileged_write(workspace):
    ...
    client.post(
        "/api/capture",
        json={"text": "watered the monstera, soil was dry", "channel": "web"},
    )
